...
vk.setOptions({ token: 'a20de8a556c5c838244ad1468f84109953a23c51e931a2db4047e07f3213557bd4d07b91aee82fadd3af8', pollingGroupId:207882008 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[club207882008\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club207882008\|(.*)\]/ig, '').trim();
...
cmd.on(/^(?:бонус|@club207882008|💎Бонус )$/i, async (message, bot) => {
...
🔹Что бы купить донат, перейдите по ссылке https://vk.com/direnyov
`)
});
...
🔹Что бы купить Валюту, перейдите по ссылке https://vk.com/direnyov
`)
});
/////
...
cmd.on(/^(?:фермы|@club207882008🔋 Ферма )\s?([0-9]+)?$/i, async (message, bot) => {
	message.user.foolder += 1;
